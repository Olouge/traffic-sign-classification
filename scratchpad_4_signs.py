...
n_train, n_test, num_channels, image_shape, labels_count, image_size, img_size_flat = compute_dimensions(X_train, X_test)
...
if a_mode == 1:
    ...
    cross_entropy = tf.reduce_mean(-tf.reduce_sum(labels * tf.log(tf.clip_by_value(prediction, 1e-10, 1.0)), reduction_indices=[1]))
