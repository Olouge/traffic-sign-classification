...
class KerasTrafficSignDataset:

    TRAFFIC_SIGN_DATA_TRAIN_SRC_PATH = 'https://d17h27t6h515a5.cloudfront.net/topher/2016/October/580ad86d_train/train.p'
    TRAFFIC_SIGN_DATA_TEST_SRC_PATH = 'https://d17h27t6h515a5.cloudfront.net/topher/2016/October/580ad89e_test/test.p'

    TRAINING_FILE = os.path.join(os.path.dirname(__file__), '..', 'traffic-sign-data', 'keras', 'train.p')
    TESTING_FILE = os.path.join(os.path.dirname(__file__), '..', 'traffic-sign-data', 'keras', 'test.p')
    PREDICTING_FILE = os.path.join(os.path.dirname(__file__), '..', 'traffic-sign-data', 'keras', 'predict.p')
    ...
    def persist(self, data, pickle_file, overwrite=False):
    ...
    def load_serialized_prediction_images(self):
        predicting_file = self.PREDICTING_FILE
    ...
    def __load_data(self):
        ...
        self.load_serialized_prediction_images()
        # predicting_file = self.PREDICTING_FILE
        # if os.path.isfile(predicting_file):
        #     with open(predicting_file, mode='rb') as f:
        #         predict = pickle.load(f)
        ...
        self.__log('Loaded traffic-sign-data/keras/train.p, traffic-sign-data/keras/test.p and traffic-sign-data/keras/predict.p')

    def __download_traffic_sign_data(self):
        """
        Downloads train and test source files and places them in their respective destination directories.
        :return:
        """
        if not os.path.isfile(self.TRAINING_FILE):
            print('{} not found. Downloading from {} now.'.format(self.TRAINING_FILE, self.TRAFFIC_SIGN_DATA_TRAIN_SRC_PATH))
            urlretrieve(self.TRAFFIC_SIGN_DATA_TRAIN_SRC_PATH, self.TRAINING_FILE)
            print('{} downloaded'.format(self.TRAINING_FILE))
        # assert hashlib.md5(open(self.TRAINING_FILE, 'rb').read()).hexdigest() == 'c2ced7c725ead1fcf7834d2ec2288c77', self.TRAINING_FILE + ' file is corrupted.  Remove the file and try again.'

        if not os.path.isfile(self.TESTING_FILE):
            print('{} not found. Downloading from {} now.'.format(self.TESTING_FILE,
                                                                  self.TRAFFIC_SIGN_DATA_TEST_SRC_PATH))
            urlretrieve(self.TRAFFIC_SIGN_DATA_TEST_SRC_PATH, self.TESTING_FILE)
            print('{} downloaded'.format(self.TESTING_FILE))
        # assert hashlib.md5(open(self.TESTING_FILE, 'rb').read()).hexdigest() == 'c2ced7c725ead1fcf7834d2ec2288c77', self.TESTING_FILE + ' file is corrupted.  Remove the file and try again.'
    ...
    def __prepare_images(self):
        ...
        self.__log(
            'Bucketized keras traffic sign images into three buckets: orig, gray and flat. flat is ' \
