...
class Cifar10Dataset:
    def __init__(self, verbose=False):
        ...
        This class contains the following mechanisms:
           ...
           1. #configure  - Trains a new network from the original Cifar10
           ...
           4. #restore    - Restores a serialized training, validation and test set to feed into another network
    ...
    def configure(self, one_hot=True, train_validate_split_percentage=0.2):
        ...
        Pipeline import sequence
          ...
          1. Reads in the classnames.csv and constructs a dictionary. The key is the class number and the value is
        ...
        if not self.__configured:
            ...
            self.class_names_map = self.__load_class_names_map()

            # I invoke each method through iteration to ensure
            # they never expect arguments for simplicity
            [f() for f in [
    ...
    def label_name(self, labels, idx):
    ...
    def restore_from_data(self, data):
        ...
        Pipeline import sequence
          ...
          2. Computes various metrics about the dataset.
          3. Computer class counts for the dataset.
        """
    ...
    def restore(self, pickle_file='cifar10_trained.pickle'):
        """
        Pipeline import sequence
    ...
    def persist(self, data, pickle_file='cifar10_trained.pickle', overwrite=False):
    ...
    def __from_data(self, data):
        if not self.__configured:
            ...
            self.class_names_map = data['class_names_map']
    ...
    def __restore(self, pickle_file='cifar10_trained.pickle'):
        if not self.__configured:
    ...
    def __load_class_names_map(self):
        map = {}
        # class_names_path = os.path.join(os.path.dirname(__file__), '..', 'signnames.csv')
        # with open(class_names_path, 'r') as class_names:
        #     has_header = csv.Sniffer().has_header(class_names.read(1024))
        #     class_names.seek(0)  # rewind
        #     incsv = csv.reader(class_names)
        #     if has_header:
        #         next(incsv)  # skip header row
        #     plots = csv.reader(class_names, delimiter=',')
        #     for row in plots:
    ...
    def load_serialized_prediction_images(self):
        predicting_file = os.path.join(os.path.dirname(__file__), '..', 'cifar10-data', 'predict.p')
    ...
    def __prepare_images(self):
        ...
        self.__log(
            'Bucketized features into three buckets: orig, gray and flat. flat is ' \
    ...
    def __str__(self):
        ...
        result.append(' ')
        result.append('Class names:')
        result.append(' ')
        result.append('  class - no. train - class name')
        result.append(' ')
        for k, v in self.class_names_map.items():
